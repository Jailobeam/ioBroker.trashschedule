...
class SourceApiAbfallIo extends BaseSource {
    ...
    async getPickupDates() {
        ...
        try {
            const selectedHouseNumber = await this.resolveHouseNumber(
                provider,
                cityId,
                districtId,
                streetId,
    ...
    async getApiHouseNumbers(provider, cityId, districtId, streetId) {
        ...
        const hasDistricts = await this.cityHasDistricts(provider, cityId);
        const districtFilter = hasDistricts && districtId ? String(districtId) : null;
    ...
    async resolveHouseNumber(provider, cityId, districtId, streetId, houseNumber) {
        if (!houseNumber) {
            return null;
        }
        ...
        return (
            houseNumbers.find(entry => String(entry.id) === String(houseNumber)) ??
            houseNumbers.find(entry => String(entry.name) === String(houseNumber)) ??
            null
        );
    }
    ...
    async getApiTypes(provider, cityId, districtId, streetId, houseNumber) {
        if (houseNumber) {
            let selectedHouseNumber = { id: houseNumber };

            if (cityId && streetId) {
                selectedHouseNumber = await this.resolveHouseNumber(
                    provider,
                    cityId,
                    districtId,
                    streetId,
            ...
            const streetsData = await this.getApiCached(
                {
                    ...
                    data: JSON.stringify({
                        ...
                        variables: { houseNumberId: String(selectedHouseNumber.id) },
                    }),
                },
                `abfallio-types-${provider}-${selectedHouseNumber.id}.json`,
            );

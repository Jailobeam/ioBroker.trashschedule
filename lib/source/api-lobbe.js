...
class SourceApiLobbe extends BaseSource {
    ...
    async clearAddressCache() {
        const files = await this.adapter.readDirAsync(this.adapter.namespace, '/');
        for (const file of files) {
            if (!file.isDir && /^lobbe-(?:states|cities-\d+|streets-\d+-\d+)\.json$/.test(file.file)) {
    ...
    async getAjaxItems(params, cacheKey) {
        ...
        if (await this.adapter.fileExistsAsync(this.adapter.namespace, cacheKey)) {
            const cached = await this.adapter.readFileAsync(this.adapter.namespace, cacheKey);
            const items = this.parseAddressItems(cached.file.toString());
            if (items !== null && items.length > 0) {
                return items;
            }
            this.adapter.log.warn(`[api-lobbe] invalid address cache ${cacheKey} - refreshing`);
        }

        // Bypass an invalid cache and only persist validated address data.
        const response = await this.getApi(config);
        const items = this.parseAddressItems(response);
        if (items === null) {
            throw new Error(`[api-lobbe] Invalid address response for ${params.action}`);
        }
        if (items.length === 0) {
            this.adapter.log.warn(`[api-lobbe] No address entries for ${params.action}, id: ${params.id ?? '-'} - not caching empty response`);
            return items;
        }
        await this.adapter.writeFileAsync(this.adapter.namespace, cacheKey, response);
        return items;
    }

    parseAddressItems(response) {
        try {
            const items = JSON.parse(response);
            if (!Array.isArray(items) || items.some(item =>
                !item || item.id == null || typeof item.text !== 'string' ||
                !item.text.trim() || /^\d{4}$/.test(item.text.trim()))) {
                return null;
            }
            return items.map(item => ({ id: item.id, name: item.text }));
        } catch {
            return null;
        }
    }

...
class SourceApiLobbe extends BaseSource {
    ...
    async getApiStreets(provider, stateId, cityId) {
        if (provider && stateId && cityId) {
            let streets = await this.getAjaxItems(
                {
                    action: 'street',
                    id: cityId,
                },
            ...
            if (this.looksLikeYearList(streets)) {
                this.adapter.log.warn(
                    `[api-lobbe] invalid cached street list detected for stateId "${stateId}" and cityId "${cityId}" - refreshing`,
                );

                try {
                    await this.adapter.delFileAsync(this.adapter.namespace, `lobbe-streets-${stateId}-${cityId}.json`);
                } catch (err) {
                    this.adapter.log.debug(`[api-lobbe] unable to delete invalid street cache: ${err}`);
                }

                streets = await this.getAjaxItems(
                    {
                        action: 'street',
                        id: cityId,
                    },
    ...
    looksLikeYearList(items) {
        return (
            Array.isArray(items) &&
            items.length > 0 &&
            items.every(item => /^\d{4}$/.test(String(item.name ?? '')))
        );
    }
